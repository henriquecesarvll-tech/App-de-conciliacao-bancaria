...
document.head.appendChild(style);
        ...
        // FUNÇÃO ULTRA-OTIMIZADA: Modal abre INSTANTANEAMENTE
        async function abrirModalConciliacao(transacaoId) {
            console.log('🚀 Abrindo modal INSTANTÂNEO para:', transacaoId);
            
            // 1. ABRIR MODAL IMEDIATAMENTE (sem esperar nada)
            document.getElementById('transacao-id').value = transacaoId;
            document.getElementById('modal-conciliacao').style.display = 'block';
            document.body.style.overflow = 'hidden';
            
            // 2. Buscar dados da transação (paralelo, não bloqueia)
            buscarDadosTransacao(transacaoId);
            
                          
                // Carregar em background
                carregarClassificacoesDoBanco(false).then(sucesso => {
                    if (sucesso) {
                        popularDropdownClassificacoes();
                        dropdown.disabled = false;
                        console.log('✅ Classificações carregadas em background!');
                    } else {
        ...
        // Função separada para buscar dados da transação (não bloqueia modal)
        async function buscarDadosTransacao(transacaoId) {
            try {
                const response = await fetch('/api/transacoes');
                const data = await response.json();
...
async def get_stats():
    ...
    try:
        ...
        # Consultar todas as transações do Supabase
        ...
        logger.info(f"✅ Stats do Supabase: {total_transacoes} total, {pendentes} pendentes, {conciliados} conciliados")
...
async def listar_transacoes(status: Optional[str] = None, banco: Optional[str] = None):
    """API para listar transações com lookup manual dos nomes"""
    try:
        if not supabase:
        ...
        logger.info(f"📊 Encontradas {len(result.data)} transações")
        
        # Fazer lookup manual dos nomes
        transacoes_processadas = []
        
        for t in result.data:
            classificacao_nome = 'Não Classificado'
            plano_nome = ''
            item_nome = ''
            
            # 🔍 BUSCAR NOME DA CLASSIFICAÇÃO
            if t.get('classificacao_id'):
                try:
                    ...
                    if classif_result.data:
                        classificacao_nome = classif_result.data[0]['nome']
                        logger.debug(f"✅ Classificação encontrada: {classificacao_nome}")
                    else:
                        logger.warning(f"⚠️ Classificação ID não encontrada: {t['classificacao_id']}")
                except Exception as e:
                    logger.error(f"❌ Erro ao buscar classificação: {str(e)}")
            
            # 🔍 BUSCAR NOME DO PLANO
            if t.get('plano_contas_id'):
                try:
                    plano_result = supabase.admin_client.table("planos_contas").select("nome").eq("id", t['plano_contas_id']).execute()
                    if plano_result.data:
                        plano_nome = plano_result.data[0]['nome']
                        logger.debug(f"✅ Plano encontrado: {plano_nome}")
                except Exception as e:
            ...
            if t.get('item_id'):
                try:
                    item_result = supabase.admin_client.table("itens").select("nome").eq("id", t['item_id']).execute()
                    if item_result.data:
                        item_nome = item_result.data[0]['nome']
                        logger.debug(f"✅ Item encontrado: {item_nome}")
                except Exception as e:
                    logger.error(f"❌ Erro ao buscar item: {str(e)}")
            
            # Montar transação com nomes corretos
            transacao = {
                'id': t['id'],
                'banco': t['banco'],
                'data': t['data'],
                ...
                'saldo': float(t['saldo']) if t['saldo'] else 0,
                'tipo_transacao': t.get('tipo_transacao'),
                'status': t['status'],
                
                # ✅ NOMES CORRETOS (não UUIDs)
                'classificacao': classificacao_nome,
                'plano_contas': plano_nome,
                'item': item_nome,
                
                'forma_pagamento': t.get('forma_pagamento'),
                'banco_origem': t.get('banco_origem'),
                'centro_custo': t.get('centro_custo'),
                'nome_recebedor': t.get('nome_recebedor'),
                'contraparte': t.get('contraparte'),
                'data_pagamento': t.get('data_pagamento'),
                'data_referencia': t.get('data_referencia'),
                'observacoes': t.get('observacoes'),
                'conciliado_em': t.get('conciliado_em'),
                'conciliado_por': t.get('conciliado_por'),
                'created_at': t.get('created_at')
            }
            
            transacoes_processadas.append(transacao)
        
        logger.info(f"✅ Processadas {len(transacoes_processadas)} transações com lookup")
        
        return {
            'success': True,
            'transacoes': transacoes_processadas,
            'total': len(transacoes_processadas)
        }
...
async def obter_classificacoes():
    ...
    try:
        ...
        cached_data = get_cache('classificacoes_estrutura')
        if cached_data:
            logger.info("✅ Usando classificações do CACHE")
            return cached_data
        
        logger.info("🔄 Cache miss - consultando banco otimizado...")
        ...
        # Armazenar no cache
        set_cache('classificacoes_estrutura', response_data)
...
async def criar_classificacao_normalizada(dados: ClassificacaoRequest):
    ...
    try:
        ...
        if result.data:
            ...
            invalidate_cache('classificacoes')
